...
void ModuleCamera::RotateCameraMouse(const iPoint& mousePosition, bool orbit) {
	...
	float xoffset = (float) mousePosition.x - lastX;
	float yoffset = (float) lastY - mousePosition.y;
	lastX = mousePosition.x;
	lastY = mousePosition.y;
...
void ModuleCamera::Zooming() {
	...
	if (zoomValue != 1.0f)
		App->options->showZoomMagnifier = true;
	else
		App->options->showZoomMagnifier = false;
}
...
void ModuleCamera::UpdatePitchYaw() {
	pitch = -math::RadToDeg(atanf(front.y / front.z));
