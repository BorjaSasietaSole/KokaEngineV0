...
GameObject::GameObject(std::string goName, const math::float4x4& parentTransform) {
	...
	sprintf_s(parentUuid, App->scene->getRoot()->uuid);
	transform = (ComponentTransform*)AddComponent(ComponentType::TRANSFORM);
...
GameObject::GameObject(const GameObject& duplicateGameObject) {
	...
	for (const auto &component : duplicateGameObject.components) {
		...
		if (duplicatedComponent->getComponentType() == ComponentType::MESH) {
			mesh = (ComponentMesh*)duplicatedComponent;
		}
		if (duplicatedComponent->getComponentType() == ComponentType::MATERIAL) {
...
void GameObject::DrawHierarchy(GameObject* goSelected) {
	...
	if (ImGui::IsItemClicked()) {
		if (App->scene->getGoSelect() != nullptr) {
			App->scene->getGoSelect()->drawGOBBox = false;
		}
	...
	if (ImGui::BeginPopup("Modify_GameObject")) {
		...
		if (ImGui::Selectable("Duplicate") && App->scene->getGoSelect() != nullptr) {
			toBeCopied = true;
		}
		if (ImGui::Selectable("Remove") && App->scene->getGoSelect() != nullptr) {
			toBeDeleted = true;
			if (App->scene->getGoSelect() == this) {
				App->scene->setGoSelected(nullptr);
			}
		}
		if (ImGui::Selectable("Move up") && App->scene->getGoSelect() != nullptr) {
			moveGOUp = true;
		}
		if (ImGui::Selectable("Move down") && App->scene->getGoSelect() != nullptr) {
			moveGODown = true;
		}
...
math::float4x4 GameObject::GetLocalTransform() const {
	...
	return float4x4::FromTRS(transform->getPosition(), transform->getRotation(), transform->getScale());
}
